...
Docker image for OM (version 9.0.0, WebRTC)
...
### to run (full) OM:
```
docker run -i --rm --network host apache/openmeetings:9.0.0
```
...
docker run -p 5443:5443 \
  ...
  -it apache/openmeetings:min-9.0.0
...
See below
```
docker build -t apache/openmeetings:min-9.0.0 .
docker push apache/openmeetings:min-9.0.0
```

### to build full:
```
docker build -t apache/openmeetings:9.0.0 --build-arg BUILD_TYPE=full .
docker push apache/openmeetings:9.0.0
```
