...
Docker image for OM (version 6.3.0, WebRTC)
...
docker run -p 5443:5443 \
  ...
  -it apache/openmeetings:min-6.3.0
...
See below
```
docker build -t apache/openmeetings:min-6.3.0 .
docker push apache/openmeetings:min-6.3.0
```

### to build full: 
```
docker build -t apache/openmeetings:6.3.0 --build-arg BUILD_TYPE=full .
docker push apache/openmeetings:6.3.0
```
