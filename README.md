...
### to run (full) OM:
```
docker run -i --rm --network host apache/openmeetings:9.1.1
```
...
docker run -p 5443:5443 \
  ...
  -it apache/openmeetings:min-9.1.1
...
See below
```
docker build -t apache/openmeetings:min-9.1.1 .
docker push apache/openmeetings:min-9.1.1
```

### to build full:
```
docker build -t apache/openmeetings:9.1.1 --build-arg BUILD_TYPE=full .
docker push apache/openmeetings:9.1.1
```
