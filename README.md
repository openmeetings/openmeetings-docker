...
Docker image for OM (version 5.0.1, WebRTC)
...
### to run (full) OM:
```
docker run -i --rm --expose=5443 --expose=8888 -p 5443:5443 -p 8888:8888 apache/openmeetings:5.0.1
```
...
docker run -p 5443:5443 \
  ...
  -it apache/openmeetings:min-5.0.1
...
See below
```
docker build -t apache/openmeetings:min-5.0.1 .
docker push apache/openmeetings:min-5.0.1
```

### to build full: 
```
docker build -t apache/openmeetings:5.0.1 --build-arg BUILD_TYPE=full .
docker push apache/openmeetings:5.0.1
```
